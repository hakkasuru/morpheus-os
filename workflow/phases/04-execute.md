...
2. Execute stage by stage, following the plan's `## Execution Order`. Per
   step, dispatch an implementer subagent per `workflow/implementer.md`
   with a self-contained step brief: the step block verbatim, the worktree
   path, the repo's conventions, and (for shared worktrees) what not to
   touch. One subagent per step. All steps of the current stage MAY be
   dispatched in parallel — the plan already guaranteed they are
