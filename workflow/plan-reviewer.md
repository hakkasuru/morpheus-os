...
- Round 2+ (the orchestrator states the round number; assume 1 when it
  doesn't): the previous review report — you overwrite it, so read it
  first and carry its findings into `## Prior findings`.
...
barred and whether any fired cap is inherent, the review round, and the
report path.
...
Classify every fired cap:

- **inherent** — a property of the task itself (security/auth/secrets/
  payment scope; a destructive or hard-to-reverse step the task cannot
  avoid). No revision can lift it; the orchestrator's loop policy goes
  straight to the human.
- **fixable** — a property of the doc (a missing or empty section, an
  unanswered question the planner can answer or route to the human,
  gate-2 deviation from the approved plan). A revision can lift it.

Set `inherent_cap: yes` when any fired cap is inherent.
