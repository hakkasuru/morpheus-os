...
Run `scripts/scorecard.sh --summary` (append the text given after this
command when the human names work ids, or drop `--summary` for per-item
rows) and relay the table. Explain the columns in one line each the first
time: auto_approve_rate is the share of gate approvals made by the
