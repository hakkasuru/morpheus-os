...
implements directly. Implementation is delegated to the implementer
subagent (`workflow/implementer.md`) — one step, one worktree, never
commits; read-only exploration to the explorer subagent
(`workflow/explorer.md`); gate reviews to the plan-reviewer subagent
(`workflow/plan-reviewer.md`); pre-delivery diff review to the
diff-reviewer subagent (`workflow/diff-reviewer.md`). Always review a
subagent's output before building on it — a subagent's report describes
what it intended to do, not necessarily what it verified.
